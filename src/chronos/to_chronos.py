# -*- coding: utf-8 -*-
"""Phase 1/6 데이터 → Chronos-2 입력(dict) 변환 (+수문 공변량 선택적 결합).
...
def station_series(wide: pd.DataFrame, station: str, cov_wide: pd.DataFrame | None = None):
    """(times, raw[wq], filled[wq], cov[filled]) 반환. cov_wide 주면 수문 공변량 결합."""
...
def build_finetune_inputs(wide, stations, target, splits, cov_wide=None, min_len=600):
    """LoRA 학습용: 각 지점 train 구간 전체를 1 item(+공변량)."""
    covs = [c for c in S.CHANNEL_ORDER if c != target]
    items, val_items = [], []
    times_all = {}
    for st in stations:
        _, raw, filled, cov = station_series(wide, st, cov_wide)
